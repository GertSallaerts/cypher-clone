...
const CREATE_UNIQUE = (syntax) => syntax.is5 ?
    `CREATE CONSTRAINT ${UNIQUE_LABEL} FOR (n:${UNIQUE_LABEL}) REQUIRE n.${UNIQUE_ID} IS UNIQUE` :
    `CREATE CONSTRAINT ON (n:${UNIQUE_LABEL}) ASSERT n.${UNIQUE_ID} IS UNIQUE`;

const DROP_UNIQUE = (syntax) => syntax.is5 ?
    `DROP CONSTRAINT ${UNIQUE_LABEL} IF EXISTS` :
